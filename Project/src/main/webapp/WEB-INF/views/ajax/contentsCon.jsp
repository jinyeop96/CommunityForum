...
<%pageContext.setAttribute("newLineChar", "\n"); %>	<%-- \n -> newLineChar로 --%>
	...
	<c:if test="${!empty files}">
		...
		<c:forEach items="${files }" var="file" >
			<tr align="right" class="board files"> 
				<td colspan="7"> 
		...
		<td align="center">
			<c:choose>
				...
				<c:when test="${empty boardRec }">
					<a href="javascript:recommend(0, ${dto.getBoard_no()}, 'board', 'like', ${page.getPage() })" id="like${dto.getBoard_no() }" class="no-deco">
					...
					<a href="javascript:recommend(0, ${dto.getBoard_no()}, 'board', 'dislike', ${page.getPage() })" class="no-deco"> 
		...
		<c:forEach items="${replyList }" var="reply">	<!--이건 게시글에 대한 답변 뿌리기 -->
			...
			<tr >
				...
				<c:if test="${reply.getReply_nickname() == nickname || !empty admin}">	<!-- 답변글과 현재 로그인한 사람이 같을 때 수정/삭제 버튼 보여줌 -->
					<a href="javascript:void(0)" onclick="showModifyBtn2(${reply.getReply_no() }, ${dto.getBoard_no() })">
			...
			<tr>
				<td colspan="7">
					...
					<c:if test="${empty replyRec }">
						<a href="javascript:void(0)"  onclick="recommend(${reply.getReply_no()}, ${dto.getBoard_no() }, 'reply', 'like', ${page.getPage() })" class="font-black no-deco">
							<img class="rec" src="<c:url value='/resources/img/logos/up1.png'/>">&nbsp;&nbsp;${reply.getReply_like() }
						</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					
						<a href="javascript:void(0)"  onclick="recommend(${reply.getReply_no()}, ${dto.getBoard_no() }, 'reply', 'dislike', ${page.getPage() })" class="font-black no-deco">
			...
			<tr id="replies${reply.getReply_no() }">
				<td colspan="7"> &nbsp;&nbsp;&nbsp;
					...
					<span id="reply_content${reply.getReply_no() }">${fn:replace(replyStr , newLineChar, "<br>")}</span> 	<!-- \n를 해주는 과정 --> 
				</td>
			</tr>
