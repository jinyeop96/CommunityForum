...
#placesList .item h5, #placesList .item .info {text-overflow: ellipsis;overflow: hidden;black-space: nowrap;}
...
$(function(){
	...
	//restaurant.jsp 들어올때 기본적으로 댓글은 숨김
	...
	var filter = "win16|win32|win64|macintel|mac|"; // PC일 경우 가능한 값
	 ...
	 if( navigator.platform){
		 if( filter.indexOf(navigator.platform.toLowerCase())<0 ){	// 모바일 접속
			...
			$("body").load("restaurantPopup.do?restaurant_search="+searchLoc,function(responseText, statusText, xhr){ 
				 if(statusText == "error")
...
function getListItem(index, places) {
    ...
    if (places.road_address_name) {
        itemStr += '    <span style="color:blue">' + places.road_address_name + '</span>' +
...
function displayInfowindow(marker, title) {
    var content = '<div style="padding:5px;z-index:1; color:black; float:center; text-align:center; vertical-align:middle; ">' + title + '</div>';
